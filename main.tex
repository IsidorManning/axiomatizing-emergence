...
\begin{definition}[Collection of Data Domains]
    The collection of admissible data domains $D$ is denoted by $\mathcal D$.
\end{definition}
...
\begin{definition}[Data Morphisms]\label{def:data-morphisms}
    A morphism $f:D\to D'$ is a measurable map (\ref{app:measurable-map})
    \[
...
Some examples of data morphisms $f:\Omega \to \Omega$ are translations, rotations, downsampling maps, cropping maps, all measurable. Some are continuous, Lipschitz with respect to the grid metric.
...
Equipping $\Omega$ with the graph distance $d(v,w)$ (\ref{app:graph-distance}), we induce a natural notion of locality. Data morphisms might be realized as graph isomorphisms (\ref{app:graph-isomorphism}), subgraph embeddings (\ref{subgraph-embeddings}), or vertices relabelings. Again, all are measurable but only some preserve graph distances; graph isomorphisms preserve full symmetry while local homomorphisms preserve neighborhoods.
...
\begin{definition}[Collection of Value Spaces]\label{def:collection-value-spaces}
    The collection of admissible value spaces $C$ is denoted by $C$.
\end{definition}
...
Let \(D = (\Omega,\Sigma_\Omega)\) be a data domain and
\(C = (V,\Sigma_C)\) a value space.
A \emph{signal} is a measurable map
\[
x : D \to C,
\]
i.e. a function \(x : \Omega \to V\) that is
\((\Sigma_\Omega,\Sigma_C)\)-measurable.
\end{definition}

Intuitively, a signal is a measurable field of values over a domain.

\begin{definition}[Signal Type]\label{def:signal-type}
Given a data domain \(D\) and value space \(C\), the \emph{signal type}
\(\Sig(D,C)\) denotes a representation schema whose realizations are
measurable spaces whose elements represent signals \(x : D \to C\).
\end{definition}
...
Let \(D_{\mathrm{in}}, D_{\mathrm{out}}\) be data domains and
\(C_{\mathrm{in}}, C_{\mathrm{out}}\) value spaces. We write
\[
X:= \Sig(D_{\mathrm{in}}, C_{\mathrm{in}}),
\quad
Y:= \Sig(D_{\mathrm{out}}, C_{\mathrm{out}})
\]
to denote the abstract input and output spaces of a learning task.
Here, \(\Sig(D,C)\) denotes a signal type whose realizations are measurable
spaces representing signals \(x : D \to C\).
\end{definition}
...
These requirements naturally lead to an algebraic–topological structure with an order $\preceq$ to express "no less than," a binary operation $\oplus$ to express accumulation ($R_1\oplus R_2=R_3$ just means that we are using $R_1$ and $R_2$ together as $R_3$), an identity element $0$ representing “no resource,” and a topology $\tau_\mathcal R$ to express smooth change and convergence. Together these define what we call the resource base, denoted $\mathcal R$. 

Intuitively, elements $R$ of $\mathcal R$ represent specific configurations of measurable resources allocated to a learning system (e.g. training tokens, model parameters, or optimization 
steps). 
...
\[
\mathcal R = (\mathcal R,\oplus,0,\preceq,\tau_\mathcal R)
\]
satisfying:
\begin{enumerate}
    \item $(\mathcal R,\oplus,0)$ is a commutative monoid,
    \item $(\mathcal R,\preceq)$ is a closed partial order,
    \item $\oplus:\mathcal R \times \mathcal R \to \mathcal R$ is monotone in each argument with respect to $\preceq$,
    \item $\tau $ is Hausdorff and makes $\oplus$ and $\preceq$ continuous.
\end{enumerate}

By abuse of notation, we use $\mathcal R$ to denote both the underlying set of resource configurations and the full structured object, when no confusion can arise.

\end{definition}

Resurces specify how much learning happens. It is countable, scalable, and accumulable. A resource base is to learning systems what a geometric domain is to signals: an abstract set where composition, comparison, and continuity are defined. The formalism 
...
Technically, we require the topology on $\mathcal R$ to be Hausdorff since limits should be unique and different resource configurations should be distinguishable. Moreover, we require continuity of $\oplus$ since accumulation should respect limits:
\[
...
The image $\psi(\mathcal R)\subseteq\mathbb R^k$ defines a concrete Euclidean realization of the abstract resource base that respect combination, ordering, and continuity. Intuitively, $\psi$ embeds the abstract resource base structure into Euclidean coordinates that respect combination, ordering, and continuity.
...
\begin{definition}[Scaling Functional]\label{def:scaling-functional}
    Given a resource chart $\psi:\mathcal R \to \R^k$, a \textit{scaling functional} $\alpha$ is a nonzero element 
    \[
    \alpha\in (\R^k)^*
    \]
...
\subsubsection{Examples: Resurces}

\paragraph{LLMs (From example \ref{exa:llms}):}

In LLMs, we might have the resource base:
\[
\mathcal R_\text{LLM}=(\R^3_{>0}, +,0,\preceq_\text{coordinate-wise}, \tau_\text{Euclidean})
\]
with coordinates:
\[
R=(T,P,C)
...
\[
\psi : \mathcal R_\text{LLM} \to \R^3,\quad \psi(T,P,C)=(\log T , \log P , \log C)
\]
...
\[
\mathcal R _\text{distil}= ( R_t \oplus R_s,\oplus, 0,\preceq, \tau)
\]
with:
\[
R_t=(T_t,P_t,C_t)
...
Let:
\[
\mathcal R_\text{steps} = (\N \cup \{\infty\},+,0,\preceq,\tau_{\text{one-point compactification of }\N})
\]
...
\end{cases}
\]
This embeds $\mathcal R_\text{steps}$ into $[0,\infty]$. Here, the direction is trivial:
\[
s(R)=\psi(R)
\]
...
\begin{definition}[Capability Equivalence of Resurces]\label{def:cap-equiv}
...
\subsubsection{The Resource Config and Resource Base Categories}

After defining the analytic structure of resources, we now introduce the categorical viewpoint that organizes these resource configurations structurally.

A resource configuration $R\in\mathcal R$ is a point in a topological, ordered, commutative monoid.
Analytically, the resource operation $\oplus$ models the combination of computational or informational resources, and the resource order $\preceq$ expresses when one configuration is no more powerful than another.

The categorical reformulation captures these ideas cleanly by turning $(\mathcal R , \preceq,\oplus)$ into a symmetric monoidal poset-category, which is precisely the type of category used across modern resource theories (quantum information, thermodynamics, computational complexity).
...
\begin{itemize}
    \item objects are resource configurations $R\in \mathcal R$, and
    \item morphisms $\Hom(R,R')$ if and only if $R\preceq R'$.
\end{itemize}
Compositionally, if morphisms $\Hom(R,R')$ and $\Hom(R',R'')$ exist, i.e., if
\[
    R\preceq R'\preceq R'',
\]
then their composition is the unique morphism $\Hom(R,R'')$.
Each object $R\in \Ob(\Res)$ has an identity morphism $\id_R:R\to R$ corresponding to reflexivity:
\[
R\preceq R.
\]
We equip $\Res$ with a symmetric structure:
\begin{enumerate}
    \item The monoidal product is the monoid operation:
    \[
    R\otimes  R' := R\oplus R'
    \]
    
\end{enumerate}
\end{definition}

This categorical structure formalizes the idea that resource configurations can only be transformed in monotone ways. Each morphism $R\to R'$ encodes an allowed “resource increase.” The category is poset because the analytic order already determines exactly which transformations exist.

The resource category itself carries little semantic content; its role is to serve as the domain over which learning dynamics and capabilities are functorially defined. All substantive claims about scaling, monotonicity, and emergence are expressed as properties of maps out of $\Res$, not of $\Res$ alone.
...
Lastly, one may also consider morphisms are functors between resource categories that preserve order, composition, and continuity, forming a category of resource categories, where objects are $\Res$ categories. Such morphisms allow comparison between different notions of resource accounting (e.g. compute-only vs compute–data models). We do not require this level of abstraction for the emergence results studied here and defer it to future work when perhaps studying more general phenomena across deep learning.
...
\begin{definition}[Parameter State Space]\label{def:param-state-space}
A \emph{parameter state space} is a measurable space
\[
(\Theta,\Sigma_\Theta),
\]
whose elements $\theta\in\Theta$ represent possible trained states produced by
training.
\end{definition}
...
\begin{definition}[Parameter Space (regularity option)]\label{def:param-space}
A \emph{parameter state space} is a measurable space
\[
(\Theta,\Sigma_\Theta),
\]
whose elements $\theta\in\Theta$ represent possible trained states produced by
training.
When convergence of probability measures on $\Theta$ is required, we assume $\Theta$ carries a topology $\tau_\Theta$ such that $\Sigma_\Theta = \mathscr B(\Theta)$. In applications, $\Theta$ is Polish (or at least standard Borel).
\end{definition}
...
\begin{definition}
    A procedure $P$ is a finite or countable specification-level object of algorithmic choices (architecture family, optimizer rule, loss functional, initialization rule, schedule, etc.) that induces a family of training kernels $\{T_P\}$ once semantics are fixed, where each
    \[
    T_P:\Prob(X\times Y)\times \mathcal R \longrightarrow \Theta
    \]
    is a Markov kernel.
\end{definition}
...
\[
R\longmapsto T _{(D,P), \ R}\in \Prob(\Theta)
\]
...
Let $D=(\Omega,\Sigma_\Omega)$ be a data domain, let
$(\Theta,\Sigma_\Theta)$ be a parameter state space, and let $P\in \mathcal P$.
...
The symbol $\Theta$ in $K=(D,\Theta,P)$ is \emph{not} an input variable to be
integrated over; it is part of the \emph{type} of the specification (the target state
space). This is analogous to how, in probability theory, one fixes a codomain
measurable space before defining random variables valued in it.
...
\[
\mathcal K \times \mathcal R\longrightarrow \theta
\]
or equivalently, as a deterministic map conditioned on randomness 
\[
(K,R,\rho)\longrightarrow \theta
\]
with $\rho\sim$ some randomness source.

Now, if we rerun training $n$ times with different seeds, the output is an empirical distribution of trained parameters. To put it in other words, "training" produces a law on parameters.

To express training as a mathematical object, we want it to take an input $(K,R)\in \mathcal K \times \mathcal R$ and output a probability distribution that is composable, measurable (so expectation make sense), and that does not assume continuity. The minimal object for this is the Markov kernel (see \ref{app:markov-kernel}).

\begin{definition}[Training Kernel]\label{def:training-kernel}
Let $\mathcal K$ be the class of admissible training specifications and let
$\mathcal R$ be a resource base, viewed at minimum as a measurable space
$(\mathcal R,\Sigma_{\mathcal R})$.
A \emph{training kernel} is a Markov kernel
\[
T:\ (\mathcal K \times \mathcal R,\ \Sigma_{\mathcal K}\otimes\Sigma_{\mathcal R})
\ \longrightarrow\ (\Theta,\Sigma_\Theta),
\]
i.e.\ a mapping $(K,R)\mapsto T_{K,R}\in\Prob(\Theta)$ such that for every
$A\in\Sigma_\Theta$ the function
\[
(K,R)\ \mapsto\ T_{K,R}(A)
\]
is measurable.
...
For $\tau=(D,\Theta,P)$ and $R\in\mathcal R$, the measure $T_{K,R}$ is the law of
the trained state $\theta$ obtained by repeatedly running the training pipeline
specified by $P$ on data from $D$ under resource allocation $R$.
...
measurable map
\[
\mathrm{Train}:\ \mathcal K \times \mathcal R \times \Omega_{\mathrm{rng}}
...
If $\Theta$ is equipped with a Polish topology $\tau_\Theta$ and $\mathcal R$ (and the
relevant part of $\mathcal K$) carry topologies that induce their measurable structures,
...
\]
in the weak topology on $\Prob(\Theta)$.
...
A trained parameter state $\theta\in\Theta$ determines how input signals are transformed into output signals. To represent this behavior in a way that accommodates both deterministic and stochastic models, we again use a Markov kernel.
...
Let $\Theta$ be a parameter space and let $Y$ be the output space.
A \emph{predictor kernel} is a Markov kernel
\[
\Pi:\ \Theta \longrightarrow \Prob(Y),
\]
assigning to each parameter state $\theta\in\Theta$ a probability measure
over output signals.
\end{definition}
...
Let $\mu\in\Prob(\Theta)$ be a probability measure over parameter states.
The \emph{mixture predictor} induced by $\mu$ is the predictor kernel
\[
\Pi_\mu : x \longrightarrow \int_\Theta \Pi_\theta(x)\, d\mu(\theta),
\]
where the integral is taken in the sense of kernel-valued integration.
\end{definition}

The mixture predictor represents the observable behavior obtained by first sampling a trained parameter state $\theta\sim\mu$, and then using the corresponding predictor $\Pi_\theta$ for inference.
Equivalently, $\Pi_\mu$ marginalizes over uncertainty in the trained model, yielding the predictive distribution induced by the training process as a whole.
...
Let $(\Theta,\Sigma_\Theta)$ be the parameter state space.
...
Morphisms in $\Pred$ act uniformly across inputs; they do not alter the
conditioning structure of predictions, only their semantic interpretation.
As such, $\Pred$ formalizes output-level invariances rather than full
functional equivalence.

This categorical structure is intentionally restrictive.
It does not model transformations of the input space, context-dependent
decoding rules, or stochastic post-processing of outputs.
Such extensions would require enlarging the morphism class to include
kernels $\mathcal Y \to \Prob(\mathcal Y)$ or functorial dependence on
$\mathcal X$, which we do not pursue here.

The role of $\Pred$ is therefore organizational rather than generative:
it provides a precise language for reasoning about when predictive
behavior should be considered equivalent for the purposes of capability
evaluation.

\paragraph{The Mixture "Functor"}

The construction of $\widehat A$ defines a map
\[
\widehat A:\Ob(\Probt)\longrightarrow\Ob(\Pred),
\]
sending parameter distributions to predictive semantics. At this stage, we do not require a nontrivial action on morphisms.

\begin{tikzpicture}[node distance=2cm] 
\node (theta) at (0,0) {$\Theta$}; 
\node (preds) at (3,0) {$\Pred$};
\node (probs) at (0,-2) {$\Probt$}; 
\node (preds2) at (3,-2) {$\Pred$}; 
\draw[->] (theta) -- node[above] {$A$} (preds); 
\draw[->] (theta) -- node[left] {$(-)_\#$} (probs); 
\draw[->] (probs) -- node[below] {$\widehat{A}$} (preds2); 
\end{tikzpicture}

This diagram expresses precisely the passage
\[
\theta\mapsto \Pi_\theta \quad \text{and}\quad \mu\mapsto \int \Pi_\theta  \ d\mu
\]

\paragraph{The Training "Functor"}

The training kernel defines a map

    \[
    K:\LS \times \Res \to \Probt
    \]

sending learning systems and resources to parameter distributions. As with the mixture map, we do not require a nontrivial action on morphism.

In summary, the categorical composite becomes:
...
This expresses categorically what training does analytically. Once this composite is defined, expected capability becomes the next categorical layer.
...
S:=(D, P)
\]
where $D\in \mathcal D$ is a data domain and $P\in \mathcal P$ is a procedure.
\end{definition}
\begin{definition}[Collection of Data Domains]
...
A learning system $S=(D,P)$ is a \emph{specification-level object}.
It does not represent a trained model, nor a random variable, nor a measurable outcome.
Instead, it fixes:
\begin{enumerate}
...
At this stage, learning systems carry no stochastic or analytical structure.
They are not assumed measurable, topological, or probabilistic objects.
Operational meaning is assigned only once resource configurations and training
semantics are introduced.

The role of a procedure $P$ is to determine \emph{how training behaves once resources
are fixed}. Formally, this behavior will later be expressed via a \emph{training kernel}
associated with $(S,R)$.
Until then, learning systems are treated purely as abstract specifications.
...
Having developed the analytic structure of learning systems — data domains, signal mappings, symmetries, procedures, distances, and neighborhoods — we now introduce the categorical perspective that organizes these objects into a coherent structural framework. The purpose is not to replace the analytic layer but to reveal and formalize the compositional relationships that already exist implicitly between the objects defined so far.
...
the theory: evaluation environments, utilities, capability types, and the associated 
...
The evaluation environment $\mathcal T$ defines the probabilistic environment in which
a capability is evaluated. 

\begin{definition}[Evaluation Environment]\label{def:evaluation-environment}
An \emph{evaluation environment} is a probability measure
\[
\mu_\text{ev} \in \Prob(X\times Y),
\]
\end{definition}

In many settings, $\mu$ is supported on pairs $(x,y)$ where $y$ is a function of $x$, but this is not assumed.

\subsubsection{Capability Scale}
...
measurable space that serves as the codomain for all state-level 
and expected capability maps.

\begin{definition}[Capability Scale]\label{def:capability-scale}
A \emph{capability scale} is a triple
\[
(\mathcal V,\preceq,\mathcal F)
\]
where:
\begin{itemize}
\item $(\mathcal V,\preceq)$ is a partially ordered set,
\item $\mathcal F$ is a $\sigma$-algebra on $\mathcal V$,
\item $\preceq$ is measurable.
\end{itemize}
\end{definition}

The codomain $\mathcal V$ represents the values returned by a capability type 
$\Phi$; the order $\preceq$ encodes ``better vs.\ worse'' performance; the measurable
structure ensures expectations such as $m_\Phi(S,R)$ are well-defined.

When studying continuity of emergence, we additionally equip $\mathcal V$ with a Hausdorff topology compatible with $\mathcal F$.
...
Given an evaluation environment $\mu$ and a capability scale $(\mathcal V,\preceq,\mathcal F)$,
a \emph{scoring rule} is a measurable function
\[
S:\Prob(\mathcal X(\Omega,C_{\mathrm{out}})) \times \mathcal X(\Omega, C_{\mathrm{out}}) \to \mathcal V.
\]
\end{definition}

The scoring rule is simply a functional mapping predictions and outcomes to capability values in the capability scale.

The scoring $S$ specifies what it means to "perform well." For 
log-likelihood, $S(y',y)=\log  p(y\mid x)$, for accuracy, 
$S(y',y)=\mathbf 1\{y'=y\}$, for reward maximization, 
$S(y',y)$ can encode a scalar reward signal. The flexibility of 
$S$ makes the framework agnostic to the specific nature of the task.
...
\begin{definition}[Capability Type]\label{def:capability-type}
    ...
    \Phi := (\mu, S, \mathcal V),
    \]
    where $\mu$ is an evaluation environment, 
    $\mathcal V$ is a capability scale, and 
    $S $ is a scoring rule.
\end{definition}
...
Given a capability type $\Phi=(\mathcal T,\mathcal U,\mathcal V_\Phi)$,
we define the evaluation functional that assigns a capability value to a predictor.

\begin{definition}[Evaluation Functional]\label{def:evaluation-functional}
Let $\mathcal T=(\Omega,C_{\mathrm{out}},\mu)$ be a learning task and
$\mathcal U$ a learning utility.
The \emph{evaluation functional} is the map
\[
E_\Phi:\ \Pred \longrightarrow \mathcal V_\Phi
\]
defined by
\[
E_\Phi(\Pi)
:= \mathbb E_{(\omega,y)\sim\mu}
\left[
\mathbb E_{y' \sim \Pi(\cdot \mid \omega)}
\bigl[\mathcal U(y',y)\bigr]
\right],
\]
...
Let $\Phi$ denote a capability type. For a predictor $\Pi_\theta:\mathcal X\to\Prob\big(\mathcal X(\Omega, C_{\mathrm{out}})\big)$ corresponding to a 
trained state $\theta\in\Theta$, the \textit{instance capability score} is the functional
\[
M_\Phi(\theta)
    :=\mathbb E_{(x,y)\sim\mu}\,\mathbb E_{y'\sim\Pi_\theta(\cdot|x)}\!\left[S(y',y)\right].
\]
Equivalently, expanding this as integrals,
\[
M_\Phi(\theta)
    =\int_{\mathcal X\times \mathcal \Prob\big(\mathcal X(\Omega, C_{\mathrm{out}})\big)}\left(\int_{\Prob\big(\mathcal X(\Omega, C_{\mathrm{out}})\big)} S (y',y)\ \Pi _\theta(dy'\mid x)\right)\mu(dx,dy).
\]
\end{definition}
...
$m_\Phi:\mathcal S \times \mathcal R \to \mathcal V$ for capability type $\Phi$ is 
\[
m_\Phi(S,R)
...
It is this map—not the loss, not the training trajectory—whose qualitative behavior under resource scaling defines emergence.

You should add a diagram or bullet list like:

Evaluation Environment → defines what world we test in

Scoring Rule → defines how outcomes are judged

Capability Scale → defines what “better” means

Instance Capability → evaluates one trained model

Expected Capability → averages over training randomness

This is the spine of the chapter.
...
\subsubsection{Categorical Packaging of Emergence}

Having defined analytically the framework for quantifying emergence—evaluation environments, capability types, instance capability, and expected capability—we now assemble these components into the categorical layer that underpins the axiomatic theory developed in the next chapter.

At the analytic level, a capability type $\Phi$ specifies a measurement scheme: it assigns a value $M_\Phi(\theta)$ to each parameter state $\theta$, and this value lies in an ordered set $V_\Phi$ (accuracy, loss, utility, etc.).
Expected capability then evaluates this map against the training distribution induced by $K$.

Categorically, this process unfolds as a chain of structure-preserving transformations: Learning systems and resources to distributions on parameters to predictors to capability values. All four stages are functorial.

\begin{definition}[$\Capa$: The Category of Capability Values]
    For a fixed capability type $\Phi$, we define the category of capability values as a poset category whose:
    \begin{itemize}
        \item values are $v\in V_\Phi$, and
        \item morphisms are arrows $\Hom(v,v')$ if and only if $v\preceq_\Phi v'$.
    \end{itemize}
    Composition is inherited from the transitivity of $\preceq _\Phi$, and each value $v$ has identity $\id_v:v\to v$ encoding reflexivity.
\end{definition}

$\Capa$ expresses the idea that capability measurements can be compared: one value is not better than another in the partial order. This categorical packaging turns monotonicity (one of your core axioms) into a structural requirement: expected capability preserves arrows. It is precisely the categorical codomain for emergent behavior: predicted capability values live here, and the functoriality of $m_\Phi$ expresses that “more resources cannot make performance worse”.
...
\begin{definition}
    Let $m_\Phi:\mathcal S\times \mathcal R \to \mathcal V$ be the capability function associated with capability type $\Phi$. Define the trivial capability floor by
    \[
...
\begin{definition}{Representability Threshold Along a Resource Axis}{}
    Let $\psi:\mathcal R \to \R^k$ be a resource chart and $\alpha\in (R^k)^*$ be an efficient direction. The \textit{representability threshold} of capability type $\Phi$ for system $S$ along direction $\alpha$ is
    \[
...
superlevel-set structure, and the existence of scaling functionals.
...
where:
\begin{enumerate}
    \item $\psi:\mathcal R\to \mathbb R^k$ is a continuous resource chart of the resource base,
...
\paragraph{Out of Scope For Now}
